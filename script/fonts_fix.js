...
Fonts_fix.prototype = {
    ...
    len: function(str) {
		...
		str = str.replace(/<\/?url>/g, "");//处理url标签，在这里
		str = str.replace(/<\/?dim>/g, "");//处理dim标签，在这里

        var len = 0;
        if (this.font_type == "yahei") {
			space_length= this.fonts_width['微软雅黑'][' '];
            for (var i = 0; i < str.length; i++) {
                len += this.fonts_width['微软雅黑'][str[i]] || this.fonts_width['微软雅黑']['yue']; //是的 - 雅黑汉字从yue获取 空格从' '得到 要的只是最终的一个倍数
            }
            len = Math.round(len / space_length); //意外的简单
        } else if (this.font_type == "tahoma") {
			space_length= this.fonts_width['Tahoma'][' '];
            for (var i = 0; i < str.length; i++) {
                len += this.fonts_width['Tahoma'][str[i]] || this.fonts_width['Tahoma']['yue'];
            }
            len = Math.round(len / space_length); //唔 另外的一个数据 大体结构已经有了 - 不需要改变太多
        } else { //别的？都当一样的处理去
